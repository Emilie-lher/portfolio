...
const experiences = [
  ...
  { 
    ...
    desc: "Enrichir le serveur cartographique avec de nouveaux styles vectoriels. Mon objectif était de créer des styles conformes à la norme SLD et optimisés pour le serveur. J'ai pu concevoir et tester des feuilles de style, développer un outil Python de débogage. Grâce à mes tests, les nouveaux styles ont été intégrés avec compatibilité et une bonne performance d’affichage. Le  souci rencontré a été que l'outil utilisé sur le serveur cartographie avait plus de restrictions sur le sld que le logiciel QGIS. Cela m'a donc obligé à créer 2 versions de mes fichiers de style au cas où il change de version sur le serveur ", 
  ...
  { 
   ...
   { 
    ...
    desc: "Crée des comptes temporaires pour les nouveaux arrivants le temps d'avoir les leurs officiels. Pour cela, il fallait automatiser la vérification et la création des comptes. J'ai donc dû comparer les données entre bases, développer une interface web. L'application n'était pas terminée à mon départ, il m'aurait fallu plus d'expérience pour la finaliser et plus de temps.", 
    tags: ["PHP", "JavaScript", "SQL"] 
  },
  { 
    denom: "Développeur Scratch / Arduino", 
    title: "Stage Orange (1 semaine)", 
    desc: "Concevoir un outil ludique pour aider les personnes avec  des prothèse. Mon objectif était de créer un jeu interactif sur Scratch intégré à un système Arduino utilisable via un joystick. Pour cela j'ai développer un mini-jeu adapté à l’interface Arduino.Le jeu est fonctionnel et  testable avec joystick. POur cela il a été important de comprendre la problématique physique des utilisateurs", 
